...
Version 0.26.37 restores immediate live activity-log loading after startup
while retaining the lower-power monitoring introduced in 0.26.36. Realtime
folders sleep in interruptible kernel
...
The following controls are enforced in 0.26.37:
...
| --- | --- | --- |
| Ubuntu/Debian | `tuxindrive_0.26.37_all.deb` | Signed in-app Debian updates remain supported. |
| Ubuntu/Debian Server | `tuxindrive-server_0.26.37_all.deb` | Separate preview service; explicit enablement, bearer token, and TLS for remote access. |
...
| Windows 10/11 x64 | `TuxInDrive-0.26.37-windows-x64-setup.exe` | Same GTK desktop UI; install WinFsp for streaming drives. |
| macOS 12+ | `TuxInDrive-0.26.37-macos-*.dmg` | Same GTK desktop UI; install macFUSE for streaming drives. |
| Android 8+ | `TuxInDrive-0.26.37-android.apk` | Native phone/tablet UI, SAF folder access and OS-managed background sync. |
...
```bash
cd ~/Downloads
sudo apt install ./tuxindrive-server_0.26.37_all.deb
```
...
The Debian installers are written to `dist/tuxindrive_0.26.37_all.deb` and `dist/tuxindrive-server_0.26.37_all.deb`. The independently versioned Network Lab build writes `dist/tuxindrive-network-lab_0.26.31+lab5_all.deb`. Windows, macOS and Android artifacts are built by `.github/workflows/platform-packages.yml` on their native build hosts. Durable product packages are attached to the matching GitHub Release; the lab uses its own `network-lab-v*` release. Dedicated signed client channel manifests and package-location pointers live under [`releases/`](releases/README.md).
