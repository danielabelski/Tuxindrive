...
Version 0.26.48 makes every visual layout follow the desktop dark-mode preference, reacts to live GNOME color-scheme changes, keeps explicit light palettes readable under dark GTK themes, and retains complete Midnight Sync dark rendering, duplicate-provider detection on both sides, non-destructive automatic cloud-duplicate renaming, verified two-way completion, safe endpoint-change pausing, persistent account renaming, drag-and-drop sidebar ordering, readable multi-line account identities across supported providers and truthful Google Drive root identity with the competitive P0/P1 reliability work and a
...
The following controls are enforced in 0.26.48:
