...
The 0.26.39 cross-folder filename index remains entirely on each desktop
...
Download `tuxindrive-server_0.26.39_all.deb` from the matching
[GitHub Release](https://github.com/tpluharik/Tuxindrive/releases/tag/v0.26.39),
...
```bash
cd ~/Downloads
sudo apt install ./tuxindrive-server_0.26.39_all.deb
```

If configuration of the defective 0.26.12 preview was left unfinished,
installing 0.26.39 replaces its launcher and completes the pending package
...
sudo apt install ./dist/tuxindrive-server_0.26.39_all.deb
```
