...
This guide covers TuxInDrive 0.26.41 on Linux, Windows, macOS and Android. Windows and macOS retain the Linux GTK desktop layout, while Android reorganizes accounts, synchronized folders, cloud files, activity and settings for touch displays. Platform-specific installation and signing details are in [Platform support](PLATFORM_SUPPORT.md). Administrators and developers can continue with the [documentation index](README.md), [operations guide](OPERATIONS.md), and [architecture reference](ARCHITECTURE.md).
...
Version 0.26.41 is the supported security and stability baseline. Upgrade older installations before reconnecting cloud or peer accounts. See [Security hardening and secure operation](SECURITY_HARDENING.md) for the complete control inventory and post-upgrade checklist.
...
- After authorization, the account card and its menu show the authenticated
  provider login (for example an email address). Existing OAuth accounts are
  filled in on the next startup. Only this display identity is saved in
  TuxInDrive metadata; OAuth tokens remain in rclone's encrypted configuration.
