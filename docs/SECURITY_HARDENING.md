...
| Area | 0.26.38 behavior | Security purpose |
|---|---|---|
...
| Filesystem writes | Relative-path validation plus descriptor-based no-follow traversal and atomic replacement for incremental downloads, deltas, recovery, hydration, and repair; vanished-parent races are ignored only for stale events/history while symlink and escape failures remain fatal | Resist traversal and symlink-swap writes outside the configured root without allowing directory moves to poison realtime retries |
...
The 0.26.38 search preview path does not change that indexing guarantee.
...
2. Confirm the running version is 0.26.38 and the platform update check reports a valid signature, origin, filename, digest, size, architecture and expiry.
